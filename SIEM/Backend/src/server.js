...
app.get('/level5', (req, res) => {
  res.sendFile(path.join(pagesPath, 'Level5.html'), (err) => {
    if (err) {
      res.status(500).send('Error serving Level5.html');
    }
  });
});
...
app.get('/level6', (req, res) => {
  res.sendFile(path.join(pagesPath, 'Level6.html'), (err) => {
    if (err) {
      res.status(500).send('Error serving Level6.html');
    }
  });
});
