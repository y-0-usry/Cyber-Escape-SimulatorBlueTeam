...
// استيراد الدوال
const { readLogFiles } = require('./core/ingestion/fileReader');
const { parseLogFile } = require('./core/parser/parser');
const { normalizeLogs } = require('./core/normalization/normalizer');
const { generateAlerts } = require('./core/Alert Generator/alertGenerator');
...
async function processAllLogsLevel(level) {
  ...
  try {
    ...
    try {
      ...
      const backendPath = path.join(__dirname, `core/Alert Generator/storage/${level}/alerts.json`);
      const frontendPath = path.join(__dirname, `../../../Frontend/src/pages/data/${level}/alerts.json`);
