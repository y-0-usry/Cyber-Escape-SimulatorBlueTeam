...
function useHint(btn, questionId) {
  ...
  if (freeHints > 0) {
    freeHints--;
    btn.textContent = '🎁 Free Hint Used!';
  } else {
    score = Math.max(0, score - 2);
    hintsUsed++;
    btn.textContent = '💡 Hint Used (-2 points)';
  }
...
function evaluatePhase1() {
  ...
  // Must get 100% to proceed to Phase 2
  if (percentage === 100) {
...
function startLevel() {
  ...
  updateScore(); // Initialize display
